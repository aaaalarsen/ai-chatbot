...
この機能により、管理画面から直接Vercelにチャットボットアプリをデプロイできます。管理画面は除外され、エンドユーザー向けのアプリのみがデプロイされます。
...
3. **Vercel設定**
   ...
   - **プロジェクト名**: デプロイするプロジェクト名（英数字とハイフン）
   - 「接続テスト」で動作確認
   - 「設定を保存」をクリック
...
- **メインアプリ**: チャットボット機能
- **音声ファイル**: 生成済みの音声データ
- **設定ファイル**: configuration.json
- **必要なコンポーネント**: Chat、Language、UI関連

### ❌ 除外されるもの
- **管理画面**: `/management` 全体
- **管理用コンポーネント**: Management関連
- **Vercel API**: デプロイ機能
- **開発用ファイル**: テスト、スクリプト等
